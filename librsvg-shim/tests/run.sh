...
# against the librsvg headers with real GLib/GIO/cairo, and runs it
# against the freshly built librsvg-2.so.2.
# Requires: a C compiler, pkg-config, the development headers for
# gobject-2.0, gio-2.0 and cairo, and the librsvg headers (only the
# headers; the system librsvg library is not used).
#
...
# Compile against the header configuration the packages ship rather
# than the system one: LIBRSVG_HAVE_PIXBUF FALSE makes rsvg.h skip
# rsvg-pixbuf.h, so this also proves no gdk-pixbuf header is needed.
rm -rf "$work/include"
mkdir -p "$work/include/librsvg-2.0"
cp -r "$rsvg_inc/librsvg" "$work/include/librsvg-2.0/"
sed -i 's/#define LIBRSVG_HAVE_PIXBUF.*/#define LIBRSVG_HAVE_PIXBUF FALSE/' \
    "$work/include/librsvg-2.0/librsvg/rsvg-features.h"
rm -f "$work/include/librsvg-2.0/librsvg/rsvg-pixbuf.h"

${CC:-cc} "$root/librsvg-shim/tests/smoke.c" -o "$work/smoke" \
    -I "$work/include/librsvg-2.0" \
    $(pkg-config --cflags --libs gobject-2.0 gio-2.0 cairo) \
